...
public class LocalLoop {
	...
	ulist = new UberList();
	this.syn_divide = ulist.getSynDivide();
	this.pos_divide = ulist.getPOSDivide();
	this.trace_divide = ulist.getTraceDivide();
	this.empties = ulist.getEmpties();
	...
	try {
            sent_loop:  do {
                ...
                if (sparse.isEmpty()) {
                    break sent_loop; }
		file_info.totalAdd1();
		per_sentence = BuildLex.OneSentence(syn_divide, 
						    pos_divide,
						    trace_divide,
						    empties);
    ...
    */
	...
	ulist.Sort();
	PrintTags.UberList(ulist, OutStuff);
	//ulist.PrintToSystemErr(8, 9);
	return; }
