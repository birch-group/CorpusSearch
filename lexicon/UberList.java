...
public class UberList {
	...
	POSPairBucket posbucket;
	LexEntryBucket lexbucket;
	POSPair paris;
	LexEntry lurk;
	int i, j, k, start_here = 0;

	try {
	    folly_loop: for (i = 0; i < folly.size(); i++) {
		posbucket = folly.bucketAt(i);
		lexbucket = this.bucketAt(i);
		if (posbucket.isEmpty()) {
		    continue folly_loop; }
		if (lexbucket.isEmpty()) {
		    lurk = new LexEntry(posbucket.POSPairAt(0));
		    lexbucket.AddLexEntry(lurk); 
		    start_here = 1;}
		for (j = start_here; j < posbucket.size(); j++) {
		    paris = posbucket.POSPairAt(j);
		    lexx_loop: for (k = 0; k < lexbucket.size(); k++) {
			lurk = lexbucket.LexEntryAt(k);
			if (lurk.belongs(paris)) {
			    lurk.addPOSPair(paris);
			    break lexx_loop; }
			if (k == lexbucket.size() - 1) {
			    lurk = new LexEntry(paris);
			    lexbucket.AddLexEntry(lurk); 
			    break lexx_loop; } } }
	    } 
	} 
