...
public class Remove extends PrintTree {
    ...
    */
	...
	try {
	    ...
	    if (previous_leaf) {
                ...
                previous_leaf = false; }
	    removed = Remove_or_not(sparse, one_item,
				    res_dex, out_file);
	    if (removed) {
		previous_leaf = true;
		return; }
		...
		if (!(sparse.IsLeafPOS(daughter))) {
		    RemoveNodeVec(sparse, next_dent, out_file,
				  daughter_dex.intValue(), res_dex); }
		else { 
		    if (j > 0 && !just_leaves) {
			IndentIt(next_dent, out_file); }
		    removed = Remove_or_not(sparse, daughter, 
					    res_dex, out_file);
		    if (!removed) {
    ...
    */
	...
	try {
		...
		if (res_bound.equals(one_item) && notDupe(res_bound)) {
		    ...
		    RemoveNodeVec(sparse, 2, out_file, sparse_dex, res_dex);
